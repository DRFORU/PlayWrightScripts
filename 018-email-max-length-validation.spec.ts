...
test(
  '#TCL018 - Verify login with an invalid email format exceeding the maximum length (255 characters) ; it should display validation message',
  async ({ page }, testInfo) => {
    ...
    await test.step('Enter EmailorMobile', async () => {
      await login.enterEmailorMobile(testData.emailMaxRangePlus1); //✅ use from testData
    });
    ...
    await test.step('Validate error message', async () => {
      ...
      const expectedText = messages.invalidEmailMaxRangePlus1; //✅ use from testData
