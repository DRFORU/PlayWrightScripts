...
test(
  '#TCL011 - Verify login with an invalid email format containing special chars(except dot) before @ using testdata_01 ; it should display validation message',
  async ({ page }, testInfo) => {
    ...
    await test.step('Enter EmailorMobile', async () => {
      await login.enterEmailorMobile(testData.specialCharBeforeAt_01); //✅ use from testData
    });
