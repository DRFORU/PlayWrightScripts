...
test(
  '#TCL026 - Verify login with an invalid email format with an invalid email domain using testdata_02 ; it should display validation message',
  async ({ page }, testInfo) => {
    ...
    await test.step('Enter EmailorMobile', async () => {
      await login.enterEmailorMobile(testData.emailWithInvalidDomain_02); //✅ use from testData
    });
    ...
    await test.step('Validate error message', async () => {
      ...
      const expectedText = messages.invalidEmailwithInvalidEmailDomain; //✅ use from testData
