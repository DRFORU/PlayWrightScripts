...
test(
  '#TCL013 - Verify login with an invalid email format containing special chars(except dot) before @ using testdata_03 ; it should display validation message',
  async ({ page }, testInfo) => {
    ...
    await test.step('Enter EmailorMobile', async () => {
      await login.enterEmailorMobile(testData.specialCharBeforeAt_03); //✅ use from testData
    });
    ...
    await test.step('Validate error message', async () => {
      ...
      const expectedText = messages.invalidEmailwithInvalidChars; //✅ use from testData
