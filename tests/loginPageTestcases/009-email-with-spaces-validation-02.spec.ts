...
test(
  '#TCL009 - Verify login with an invalid email format containing spaces using testdata_03 ; it should display validation message',
  async ({ page }, testInfo) => {
    ...
    await test.step('Enter EmailorMobile', async () => {
      await login.enterEmailorMobile(testData.withSpaces_03); //✅ use from testData
    });
