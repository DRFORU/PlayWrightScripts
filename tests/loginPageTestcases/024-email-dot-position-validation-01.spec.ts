...
test(
  '#TCL024 - Verify login with an invalid email format with dot position issues using testdata_02 ; it should display validation message',
  async ({ page }, testInfo) => {
    ...
    await test.step('Enter EmailorMobile', async () => {
      await login.enterEmailorMobile(testData.emailWithDotPositionIssues_02); //✅ use from testData
    });
