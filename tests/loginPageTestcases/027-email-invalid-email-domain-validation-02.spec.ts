...
test(
  '#TC027 - Verify login with an invalid email format with an invalid email domain using testdata_03 ; it should display validation message',
  async ({ page }, testInfo) => {
    ...
    await test.step('Enter EmailorMobile', async () => {
      await login.enterEmailorMobile(testData.emailWithInvalidDomain_03); //✅ use from testData
    });
