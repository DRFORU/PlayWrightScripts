...
test(
  '#TCL015 - Verify login with an invalid email format containing special chars(except dot) after @ using testdata_02 ; it should display validation message',
  async ({ page }, testInfo) => {
    ...
    await test.step('Enter EmailorMobile', async () => {
      await login.enterEmailorMobile(testData.specialCharAfterAt_02); //✅ use from testData
    });
