...
test(
  '#TCL010 - Verify login with an invalid email format containing spaces using testdata_04 ; it should display validation message',
  async ({ page }, testInfo) => {
    ...
    await test.step('Enter EmailorMobile', async () => {
      await login.enterEmailorMobile(testData.withSpaces_04); //✅ use from testData
    });
    ...
    await test.step('Validate error message', async () => {
      ...
      const expectedText = messages.invalidEmailWithSpaces; //✅ use from testData
