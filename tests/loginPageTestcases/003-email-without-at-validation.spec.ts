...
test(
  '#TCL003 - Verify login with an invalid email format without @ using testdata_01; it should display validation message',
  async ({ page }, testInfo) => {
    ...
    await test.step('Enter EmailorMobile', async () => {
      await login.enterEmailorMobile(testData.withoutAt_01); //✅ use from testData
    });
    ...
    await test.step('Validate error message', async () => {
      ...
      const expectedText = messages.invalidEmailWithoutAt; //✅ use from testData
