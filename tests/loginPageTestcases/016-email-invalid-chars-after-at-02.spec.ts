...
test(
  '#TCL016 - Verify login with an invalid email format containing special chars(except dot) after @ using testdata_03 ; it should display validation message',
  async ({ page }, testInfo) => {
    ...
    await test.step('Enter EmailorMobile', async () => {
      await login.enterEmailorMobile(testData.specialCharAfterAt_03); //✅ use from testData
    });
