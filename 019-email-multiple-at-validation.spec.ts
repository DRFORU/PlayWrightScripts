...
test(
  '#TCL019 - Verify login with an invalid email format with multiple @ using testdata_01 ; it should display validation message',
  async ({ page }, testInfo) => {
    ...
    await test.step('Enter EmailorMobile', async () => {
      await login.enterEmailorMobile(testData.emailWithMultipleAt_01); //✅ use from testData
    });
    ...
    await test.step('Validate error message', async () => {
      ...
      const expectedText = messages.invalidEmailWithMultipleAt; //✅ use from testData
