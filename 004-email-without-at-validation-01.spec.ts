...
test(
  '#TCL004 - Verify login with an invalid email format without @ using testdata_02 ; it should display validation message',
  async ({ page }, testInfo) => {
    ...
    await test.step('Enter EmailorMobile', async () => {
      await login.enterEmailorMobile(testData.withoutAt_02); //✅ use from testData
    });
