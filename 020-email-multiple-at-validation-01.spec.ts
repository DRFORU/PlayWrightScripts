...
test(
  '#TCL020 - Verify login with an invalid email format with multiple @ using testdata_02 ; it should display validation message',
  async ({ page }, testInfo) => {
    ...
    await test.step('Enter EmailorMobile', async () => {
      await login.enterEmailorMobile(testData.emailWithMultipleAt_02); //✅ use from testData
    });
