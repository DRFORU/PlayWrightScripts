...
test(
  '#TCL005 - Verify login with an invalid email format without dot using testdata_01 ; it should display validation message',
  async ({ page }, testInfo) => {
    ...
    await test.step('Enter EmailorMobile', async () => {
      await login.enterEmailorMobile(testData.withoutDotInDomain_01); //✅ use from testData
    });
