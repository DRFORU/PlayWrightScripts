...
test(
  '#TCL017 - Verify login with an invalid email format less than the minimum length (5 characters) ; it should display validation message',
  async ({ page }, testInfo) => {
    ...
    await test.step('Enter EmailorMobile', async () => {
      await login.enterEmailorMobile(testData.emailMinRangeMinus1); //✅ use from testData
    });
    ...
    await test.step('Validate error message', async () => {
      ...
      const expectedText = messages.invalidEmailMinRangeMinus1; //✅ use from testData
