...
test(
  '#TCL014 - Verify login with an invalid email format containing special chars(except dot) after @ using testdata_01 ; it should display validation message',
  async ({ page }, testInfo) => {
    ...
    await test.step('Enter EmailorMobile', async () => {
      await login.enterEmailorMobile(testData.specialCharAfterAt_01); //✅ use from testData
    });
