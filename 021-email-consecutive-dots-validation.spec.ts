...
test(
  '#TCL021 - Verify login with an invalid email format with consecutive dots using testdata_01 ; it should display validation message',
  async ({ page }, testInfo) => {
    ...
    await test.step('Enter EmailorMobile', async () => {
      await login.enterEmailorMobile(testData.emailWithConsecutiveDots_01); //✅ use from testData
    });
